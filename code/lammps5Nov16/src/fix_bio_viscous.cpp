...
FixBioViscous::FixBioViscous(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  gamma(NULL)
{
  if (narg < 4) error->all(FLERR,"Illegal fix viscous command");

  gamma_one = force->numeric(FLERR,arg[3]);
  gamma = memory->create(gamma,atom->ntypes+1,"bio/viscous:gamma");
...
void FixBioViscous::setup(int vflag)
{
...
void FixBioViscous::min_setup(int vflag)
{
  post_force(vflag);
}
...
void FixBioViscous::post_force(int vflag)
{
...
void FixBioViscous::post_force_respa(int vflag, int ilevel, int iloop)
{
...
void FixBioViscous::min_post_force(int vflag)
{
  post_force(vflag);
}
