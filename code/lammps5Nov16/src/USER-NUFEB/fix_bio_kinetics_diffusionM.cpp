...
FixKineticsDiffusionM::FixKineticsDiffusionM(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg)
{
...
FixKineticsDiffusionM::~FixKineticsDiffusionM()
{
  int i;
  for (i = 0; i < 1; i++) {
    delete [] var[i];
...
int FixKineticsDiffusionM::setmask()
{
  int mask = 0;
  mask |= PRE_FORCE;
  return mask;
...
void FixKineticsDiffusionM::init()
{
...
SparseMatrix<double> FixKineticsDiffusionM::laplacian_matrix_3d()
{
  VectorXi ex1(nx);
  ex1.setOnes();
  MatrixXi  ex (nx, 3);
...
SparseMatrix<double> FixKineticsDiffusionM::spdiags(MatrixXi& B, VectorXi& d, int m, int n, int size_d)
{
...
bool* FixKineticsDiffusionM::diffusion(bool *nuConv, int iter, double diffT)
{
...
VectorXd FixKineticsDiffusionM::bc_vec(VectorXd& S, double h)
{
  VectorXd B(ngrids);
  B.setZero();
  int i_m;
...
bool FixKineticsDiffusionM::isEuqal(double a, double b, double c)
{
