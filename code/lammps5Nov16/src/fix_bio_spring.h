...
FixStyle(bio/spring,FixBioSpring)

#else

#ifndef LMP_FIX_BIO_SPRING_H
#define LMP_FIX_BIO_SPRING_H

#include "fix.h"

namespace LAMMPS_NS {

class FixBioSpring : public Fix {
 public:
  FixBioSpring(class LAMMPS *, int, char **);
  ~FixBioSpring();
  int setmask();
  void init();
  void setup(int);
 ...
 private:
  ...
  double cd, twitch, motility, detach;
  int tflag;
