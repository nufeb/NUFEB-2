...
FixDivide::FixDivide(LAMMPS *lmp, int narg, char **arg) :
  ...
  if (narg != 7)
    error->all(FLERR, "Illegal fix divide command: Missing arguments");

  nevery = force->inumeric(FLERR, arg[3]);
  if (nevery < 0)
    error->all(FLERR, "Illegal fix divide command: calling steps should be positive integer");
  ...
  if (seed <= 0)
    error->all(FLERR, "Illegal fix divide command: seed should be greater than 0");
...
void FixDivide::post_integrate() {
  ...
  for (i = 0; i < nall; i++) {
    if (atom->mask[i] == avec->maskEPS || atom->mask[i] == avec->maskDEAD)
      continue;

    if ((atom->mask[i] & groupbit) && atom->x[i][0] >= sublo[0] && atom->x[i][0] < subhi[0] && atom->x[i][1] >= sublo[1]
        && atom->x[i][1] < subhi[1] && atom->x[i][2] >= sublo[2] && atom->x[i][2] < subhi[2]) {

      density = atom->rmass[i] / (4.0 * MY_PI / 3.0 * atom->radius[i] * atom->radius[i] * atom->radius[i]);
