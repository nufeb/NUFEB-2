/* ----------------------------------------------------------------------
F LAMMPS - Large-scale Atomic/Molecular Massively Parallel Simulator
...
#include "fix_bio_kinetics.h" // added JS
...
void lammps_put_local_info(void* ptr, int nLocalIn, double* fdrag, double* DuDt,
    int* foamCpuIdIn, int* tagIn, double* gridPU) {
  ...
  for (i = 0; i < (lammps->modify->nfix); i++) {
    if (strcmp(lammps->modify->fix[i]->style, "fdrag") == 0) {
      drag_ptr = (FixFluidDrag *) lammps->modify->fix[i];
    } else if (strcmp(lammps->modify->fix[i]->style, "kinetics") == 0) {
  ...
  if (nLocalIn != nlocal) {
    ...
    printf("Incoming drag is: %5d, local particle number is: %5d.", nLocalIn,
        nlocal);
  }
...
void lammps_step_stop(void *ptr, int n, int nsteps) {
  ...
  nc = sprintf(cmd, "run %d pre no post no stop %d", n, nsteps);
  if (nc <= 0) {
...
void lammps_create_particle(void* ptr, int npAdd, double* position, double* tag,
  ...
  for (int i = 0; i < lammps->atom->nlocal; i++)
    max = MAX(max, tagOld[i]);
  ...
  for (int m = 0; m < npAdd; m++) {
    ...
    lammps->atom->rmass[n] = 4.0 * 3.14159265358917323846 / 3.0 * radtmp
        * radtmp * radtmp * rho;
