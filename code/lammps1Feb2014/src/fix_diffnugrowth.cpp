...
void FixDiffNuGrowth::change_dia()
{
  ...
  for (int j = 0; j < numCells; j++) {
      caseOneOne[j] = update->dt * (R1[j] + R4[j] + R5[j] - R6[j] - R10[j] - R13[j] - R14[j]);
      caseOneTwo[j] = update->dt * (YEPS/YHET) *(R1[j] + R4[j] + R5[j]);
      caseTwo[j] = update->dt * (R2[j] - R7[j] - R11[j]);
