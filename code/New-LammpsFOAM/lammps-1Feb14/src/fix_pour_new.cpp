...
FixPourNew::FixPourNew(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  ...
  memory->create(coords,natom,4,"pourNew:coords");
  memory->create(imageflags,natom,"pourNew:imageflags");
...
void FixPourNew::pre_exchange()
{
  ...
  double **xmine,**xnear;
  memory->create(xmine,ncount,4,"fix_pour_new:xmine");
  memory->create(xnear,nprevious+nnew*natom,4,"fix_pour_new:xnear");
  int nnear = nprevious;
...
int FixPourNew::outside(int dim, double value, double lo, double hi)
{
...
void FixPourNew::xyz_random(double h, double *coord)
{
  if (domain->dimension == 3) {
    if (region_style == 1) {
...
double FixPourNew::radius_sample()
{
...
void FixPourNew::options(int narg, char **arg)
{
  // defaults
...
void *FixPourNew::extract(const char *str, int &itype)
{
