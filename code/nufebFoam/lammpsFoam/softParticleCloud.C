...
void softParticleCloud::initLammps() {
  ...
  lamString = string("Inputscript.lammps");
  N = lamString.length();
  ...
  Info << "Reading Lammps inputfile (Inputscript.lammps) ..." << endl;
  ...
  Info << "FOAM reported # of particles according to Lammps: " << nGlobal_
      << endl;
  ...
  lammps_get_initial_info(lmp_, xArray_, vArray_, dArray_, rhoArray_, tagArray_,
      lmpCpuIdArray_, typeArray_);

  Info << "constructing particles..." << endl;
  Info << "execution time is: " << runTime_.elapsedCpuTime() << endl;

  initConstructParticles(nLocal, xArray_, vArray_, dArray_, rhoArray_,
      tagArray_, lmpCpuIdArray_, typeArray_);
...
//  Info << "the number of particles in LAMMPS now is: " << lmpNGlobal << endl;
  Info << "the number of particles in OpenFOAM now is: " << size() << endl;
  // deleteAllParticleOF();
  ...
  if (size() != lmpNGlobal) {
    ...
    lammps_get_local_info(lmp_, fromLmpXArrayLocal, fromLmpVArrayLocal,
...
//    Cloud < softParticle > ::move(td0, 0);
    ...
    )
    {
        ...
        {
            softParticle& p = pIter();
            if (fromLmpTagArrayLocal[i] == p.ptag()) {
              del = 0;
              break;
            }
        }
    ...
    Info << "the number of particles in OpenFOAM after update is: " << size() << endl;
...
void softParticleCloud::adjustLampTimestep() {
  ...
  solidStepsPerDt_ = (int(solidStepsPerDt_) / int(subCycles_))
      * int(subCycles_);
  ...
  if (subCycles_ >= solidStepsPerDt_) {
    ...
    if (nXtra_ != 0)
      FatalErrorIn("softParticleCloud::adjustLampTimestep() ")
          << "Time step adjustment error. " << "Subcycles: " << subCycles_
          << "Steps per cycle: " << subSteps_ << "Extra steps: " << nXtra_
          << abort(FatalError);
  }
  ...
  Info << "Lammps timestep adjusted from " << dtLampIn << " to " << dtLampAdj
      << endl;
  Info << "Adjusted according to: " << int(solidStepsPerDt_)
      << " solid steps per fluid step." << endl;
  Info << "Sub-cycle report: " << subCycles_ << " sub-cycles with " << subSteps_
      << " steps per cycle and " << nXtra_ << " extra steps." << endl;
}

// Construct particles in FOAM from Lammps data:
void softParticleCloud::initConstructParticles(int nLocal, double* x, double* v,
    double* d, double* rho, int* tag, int* lmpCpuId, int* type) {
  // if (Pstream::master())
  {
    int offset;
    for (int i = 0; i < nLocal; i++) {
      ...
      vector velo = vector(v[offset + 0], v[offset + 1], v[offset + 2]);
    ...
    label i = 0;
    for (softParticleCloud::iterator pIter = begin(); pIter != end();
        ++pIter, ++i) {
      offset = 3 * i;

      vector pos = vector(x[offset + 0], x[offset + 1], x[offset + 2]);
...
void softParticleCloud::assembleList(DataType& inList, //attribute of each OF local atom
    List<DataType>& outList, labelList listSizeList, // atom size in each LP proc
    labelList cpuIdList // LP cpu id of each OF local atom
    ) {
  label nprocs = Pstream::nProcs();
...
template<class DataType>
void softParticleCloud::flattenList(List<DataType>& inList, DataType& outList) {
  int i = 0;
  forAll(inList, listI)
  {
...
softParticleCloud::softParticleCloud(const volVectorField& U,
    const volScalarField& p, volVectorField& Ue, dimensionedScalar nu,
    volScalarField& alpha, IOdictionary& cloudProperties) :
    Cloud<softParticle>(U.mesh(), "softParticleCloud"), nGlobal_(0), runTime_(
        U.time()), mesh_(U.mesh()), nu_(nu), cloudProperties_(cloudProperties), Ue_(
        Ue), U_(U), pf_(p), gamma_(alpha), cpuTimeSplit_(6, 0.0) {
  label nprocs = Pstream::nProcs();

  subCycles_ = readScalar(cloudProperties_.lookup("subCycles"));

  transposeNbrOnly_ = cloudProperties_.lookupOrDefault < Switch
      > ("transposeNbrOnly", false);
  ...
  if (addParticleOption_ > 0) {
    addParticleTimeStep_ = readScalar(
        cloudProperties_.lookup("addParticleTimeStep"));
    randomPerturb_ = readScalar(cloudProperties_.lookup("randomPerturb"));
    addParticleBox_ = cloudProperties_.lookupOrDefault("addParticleBox",
        tensor::zero);
    addParticleBoxEccentricity_ = cloudProperties_.lookupOrDefault(
        "eccentricity", vector::zero);
    addParticleInfo_ = cloudProperties_.lookup("addParticleInfo");
    addParticleVel_ = cloudProperties_.lookupOrDefault("addParticleVelocity",
        vector::zero);
    inletBox_ = cloudProperties_.lookupOrDefault("inletBox", tensor::zero);
    reduceNumberFactor_ = cloudProperties_.lookupOrDefault("reduceNumberFactor",
        1);
  ...
  if (deleteParticleOption_ > 0) {
    deleteParticleBox_ = cloudProperties_.lookupOrDefault("deleteParticleBox",
        tensor::zero);
    totalDelete_ = 0;
  } else {
  ...
  if (deleteBeforeAddFlag_ > 0) {
    clearInitialBox_ = cloudProperties_.lookupOrDefault("clearInitialBox",
        tensor::zero);
...
void softParticleCloud::setPositionVeloCpuId(std::vector<vector> &XLocal,
    std::vector<vector> &VLocal, std::vector<int> &lmpCpuIdLocal) {
  int i = 0;
  nLocal_ = 0;
  for (softParticleCloud::iterator pIter = begin(); pIter != end();
      ++pIter, ++i) {
    ...
    nLocal_++;
    //Pout<< "particle2 is:" << i << ", x = "<< XLocal[i] << "  v = "<<  p.U() << "  lamID = " << p.pLmpCpuId()<< " old u = " <<  p.UOld()<<  endl;
  }
...
template<class DataType>
void softParticleCloud::transposeAmongProcs(List<DataType>& toEveryone,
    List<DataType>& fromEveryone) {
  label nprocs = Pstream::nProcs();
  label myrank = Pstream::myProcNo();

  if (toEveryone.size() != nprocs) {
    FatalErrorIn("softParticleCloud::transAmongProcs() ")
  ...
  if (transposeNbrOnly_) {
    ...
    forAll(patches, patchI)
    {
      ...
      if (isA < processorPolyPatch > (pp)) {
        const processorPolyPatch& procPatch = refCast<const processorPolyPatch>(
            pp);
...
// the job.
void softParticleCloud::lammpsEvolveForward(std::vector<vector> &XLocal,
    std::vector<vector> &VLocal, std::vector<int> &lmpCpuIdLocal,
    vectorList FLocal, vectorList DuDtLocal, int nstep,
  ...
  assembleList<vectorList>(fromFoamDragList, fromFoamDragListList,
      lmpParticleNo, assembleLmpCpuIdList);

  assembleList<vectorList>(fromFoamDuDtList, fromFoamDuDtListList,
      lmpParticleNo, assembleLmpCpuIdList);

  assembleList<labelList>(fromFoamFoamCpuIdList, fromFoamFoamCpuIdListList,
      lmpParticleNo, assembleLmpCpuIdList);

  assembleList<labelList>(fromFoamTagList, fromFoamTagListList, lmpParticleNo,
  ...
  transposeAmongProcs<labelList>(fromFoamFoamCpuIdListList,
...
//    for (int i = 0; i < UfSmoothed_.size() ; i++){
//    	Pout << toLmpCellPUArray_[6*i] << ", " << toLmpCellPUArray_[6*i + 1] << ", "
//    			<< toLmpCellPUArray_[6*i + 2] << ", " << toLmpCellPUArray_[6*i + 3] << ", " << toLmpCellPUArray_[6*i + 4] << ", " << toLmpCellPUArray_[6*i + 5] << ", " << endl;
//    }
  ...
  if (contiguous<vector>()) {
    ...
    int* toLmpTagLocalArray_ = reinterpret_cast<int *>(&(toLmpTagList.first()));

    lammps_put_local_info(lmp_, toLmpListSize, toLmpFLocalArray_,
        toLmpDuDtLocalArray_, toLmpFoamCpuIdLocalArray_, toLmpTagLocalArray_,
        toLmpCellPUArray_);
  } else {
  ...
  lmpCpuIdLocal.resize(lmpNLocal);
  ...
  // nufeb code
  labelList fromFoamTagNewList(nList, 0);
  ...
  for (softParticleCloud::iterator pIter = begin(); pIter != end();
    ...
    fromFoamTagNewList[j] = p.ptag();
  }

  std::sort(fromFoamTagNewList.begin(), fromFoamTagNewList.end());

  assembleList<vectorList>(fromLmpXList, fromLmpXListList, foamParticleNo,
      fromLmpFoamCpuIdList);

  assembleList<vectorList>(fromLmpVList, fromLmpVListList, foamParticleNo,
      fromLmpFoamCpuIdList);

  assembleList<labelList>(fromLmpFoamCpuIdList, fromLmpFoamCpuIdListList,
      foamParticleNo, fromLmpFoamCpuIdList);

  assembleList<labelList>(fromLmpLmpCpuIdList, fromLmpLmpCpuIdListList,
      foamParticleNo, fromLmpFoamCpuIdList);

  assembleList<labelList>(fromLmpTagList, fromLmpTagListList, foamParticleNo,
      fromLmpFoamCpuIdList);
  ...
  transposeAmongProcs<labelList>(fromLmpFoamCpuIdListList,
      toFoamFoamCpuIdListList);
  transposeAmongProcs<labelList>(fromLmpLmpCpuIdListList,
  ...
  sortedOrder(fromFoamTagNewList, sortedFromFoamTag);
  ...
  for (label i = 0; i < nList; i++) {
    ...
    // position
    XLocal[fromI] = vector(toFoamXList[toI].x(), toFoamXList[toI].y(),
        toFoamXList[toI].z());

    // velocity
    VLocal[fromI] = vector(toFoamVList[toI].x(), toFoamVList[toI].y(),
        toFoamVList[toI].z());

    // lmpCpuId
    lmpCpuIdLocal[fromI] = toFoamLmpCpuIdList[toI];
    //Pout<< "fromI " << fromI << ", toI = "<< toI <<  endl;
    // Pout<< "particle is:" << i << ", x = "<< XLocal[i] << "  v = "<< VLocal[i] << "  lamID = " << lmpCpuIdLocal[i]<<  endl;
  }
...
template<typename T>
void softParticleCloud::resize(T*& arr, int newSize, int oldSize) {
  T* ret = new T[newSize]();

  memcpy(ret, arr, oldSize * sizeof(T));
...
void softParticleCloud::addNewParticles() {
  ...
  assembleList<vectorList>(fromFoamAddPositionList, fromFoamAddPositionListList,
      addedParticleNo, assembleLmpCpuIdList);

  assembleList<labelList>(fromFoamAddTagList, fromFoamAddTagListList,
      addedParticleNo, assembleLmpCpuIdList);

  List < vectorList > toLmpAddPositionListList(nprocs);
  List < labelList > toLmpAddTagListList(nprocs);

  transposeAmongProcs<vectorList>(fromFoamAddPositionListList,
      toLmpAddPositionListList);
  ...
  {
    posArray[0 + 3 * i] = toLmpAddPositionList[i].x()
        + randomPerturb_ * (0.5 - perturbation.scalar01());
    posArray[1 + 3 * i] = toLmpAddPositionList[i].y()
        + randomPerturb_ * (0.5 - perturbation.scalar01());
    posArray[2 + 3 * i] = toLmpAddPositionList[i].z()
        + randomPerturb_ * (0.5 - perturbation.scalar01());
...
//- Update OpenFOAM particles
void softParticleCloud::deleteAllParticleOF() {
  Info << "Remove all particels from OpenFOAM" << endl;
...
bool softParticleCloud::pointInRegion(vector& point, tensor& box) {
  ...
  if (addParticleOption_ == 1) {
    if ((point.x() - x1) * (point.x() - x2) < ROOTVSMALL
