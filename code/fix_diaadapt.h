...
FixStyle(diaadapt,FixDiaAdapt)

#else

#ifndef LMP_FIX_DIAADAPT_H
#define LMP_FIX_DIAADAPT_H

#include "fix.h"

namespace LAMMPS_NS {

class FixDiaAdapt : public Fix {
 public:
  // int diamflag;        // 1 if atom diameters will vary, for AtomVecGranular
  // int chgflag; 

  FixDiaAdapt(class LAMMPS *, int, char **);
  ~FixDiaAdapt();
  int setmask();
 ...
 private:
  ...
  // int ndiaadapt,resetflag,scaleflag;
  // int anypair;
  // int nlevels_respa;
