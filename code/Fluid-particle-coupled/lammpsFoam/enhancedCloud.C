...
//- Update particle alpha list (per fluid step)
void  enhancedCloud::updateParticleSauterDiameter()
{
    pAveD_.setSize(particleCount_);
    label particleI = 0;
    for
    (
        softParticleCloud::iterator pIter = softParticleCloud::begin();
...
void  enhancedCloud::updateDragOnParticles()
{
    ...
    setupParticleDia();
    updateParticleAlpha();
    updateParticleSauterDiameter();
    Info<< "Debug after updateParticleSauterDiameter()" << endl;
	...
	//        Info<< "correcFac true 1" << endl;
		// local pressure gradient is used though (no weighting).

		  if(pAveD_[particleI] < scalar(1.0e-6)){
		     pDrag_[particleI] =  Jd_[particleI]*(1.0 - pAlpha_[particleI])*p.Vol()*Uri_[particleI]- gradp[p.cell()]*p.Vol(); 
	  //           Info<< "correcFac true pdrag1" << pDrag_[particleI] << endl;
		  }else{
		     pDrag_[particleI] =
		    ((1.0 - pAlpha_[particleI])*p.d()/pAveD_[particleI] + pAlpha_[particleI]*pow(p.d()/pAveD_[particleI],2) + 0.064*(1.0 - pAlpha_[particleI])*pow(p.d()/pAveD_[particleI],3))
		*Jd_[particleI]*(1.0 - pAlpha_[particleI])
		   *p.Vol()*Uri_[particleI]         // Drag
		  - gradp[p.cell()]*p.Vol();        // Buoyancy
	  //           Info<< "correcFac true pdrag2" << pDrag_[particleI] << endl;
	  //           Info<< "correcFac true pdrag2 aveD_" << pAveD_[particleI] << endl;
	  //           Info<< "correcFac true pdrag2 corrfac" << ((1.0 - pAlpha_[particleI])*p.d()/pAveD_[particleI] + pAlpha_[particleI]*pow(p.d()/pAveD_[particleI],2) + 0.064*(1.0 - pAlpha_[particleI])*pow(p.d()/pAveD_[particleI],3)) << endl;
		  }
	 //         Info<< "correcFac true 2" << endl;
		}
	    }
	}
	...
	void enhancedCloud::calcTcFields()
	{
	    ...
	    updateParticleAlpha();
            updateParticleSauterDiameter(); //added 
	    updateParticleUr();
	    setupParticleDia();
	...
	//Info<< "Debug 71b" << endl;
	      Jd_ = drag2_->Jd(magUri_, pAveD_); //my guess: aveD_ is no transformed from volScalarField to scalarField. check that
	    }
	//Info<< "Debug 72" << endl;
    ...
    if(poly == false){
      ...
      drag2_ = Foam::polyDragModel::New(cloudDict, transDict, pAlpha_, pDia_, pAveD_);
    }
...
void enhancedCloud::particleToEulerianField()
{
    ...
    xi0.internalField() *= 0.0;
    xi1.internalField() *= 0.0;
    scalar d0 = 0.0025;
    scalar d1 = 0.0015;
//    scalarField ni0 *= 0.0;
//    scalarField ni1 *= 0.0;
    aveD.internalField() *= 0.0;
//        Info<< "Debug 1" << endl;
...
/*
        if(p.ptype() == 1){
           xi0.internalField()[cellI] += p.Vol();
//           d0 = p.d();
        }
        else{
           xi1.internalField()[cellI] += p.Vol();
//           d1 = p.d();
        }
*/
        ...
        if(p.ptype() == 1){
//           ni0[cellI] += 1;
           xi0.internalField()[cellI] += 1.0;
        }
        else if(p.ptype() == 2){
//           ni1[cellI] += 1;
           xi1.internalField()[cellI] += 1.0;
        }
...
/*    
        Info<< "Debug 1" << endl;
        xi0.internalField() = xi0.internalField()/max(gamma_.internalField(), scalar(1.0e-6));
  //      Info<< "Debug 2" << endl;
        xi1.internalField() = xi1.internalField()/max(gamma_.internalField(), scalar(1.0e-6));
  //      Info<< "Debug 3" << endl;

        // averaged diameter (Sauter diameter)
    forAll(aveD_.internalField(), cI){
        if (xi0.internalField()[cI]/d0 + xi1.internalField()[cI]/d1 < scalar(1.0e-6)){
           aveD_.internalField()[cI] *= 0.0;
        }else{
           aveD_.internalField()[cI] = pow(xi0.internalField()[cI]/d0 + xi1.internalField()[cI]/d1, -1); 
        }
        // todo smoothing is missing, should be perhaps added
 //       Info<< "Debug 4" << endl;
    }
*/

//    forAll(aveD_.internalField(), cI){
//       aveD_.internalField()[cI] = (ni0[cI]*pow(d0,3) + ni1[c*pow(d1,3))/(ni0*pow(d0,2) + ni1*pow(d1,2));

    forAll(aveD.internalField(), cI){
       if((xi0.internalField()[cI]*pow(d0,2) + xi1.internalField()[cI]*pow(d1,2)) < scalar(1.0e-6)){
//           aveD.internalField()[cI] = (xi0.internalField()[cI]*pow(d0,3) + xi1.internalField()[cI]*pow(d1,3))/(xi0.internalField()[cI]*pow(d0,2) + xi1.internalField()[cI]*pow(d1,2));
           aveD.internalField()[cI] = 0.0;
       }else{
           aveD.internalField()[cI] = (xi0.internalField()[cI]*pow(d0,3) + xi1.internalField()[cI]*pow(d1,3))/(xi0.internalField()[cI]*pow(d0,2) + xi1.internalField()[cI]*pow(d1,2));
       }
    }
...
//        Info<< "Debug 2" << endl;
    ...
    aveD.correctBoundaryConditions();
}
