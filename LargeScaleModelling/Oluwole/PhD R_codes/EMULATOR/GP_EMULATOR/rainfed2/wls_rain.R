...
sfLibrary(mlegp)
if(length(u)==0){
fitPC = mlegp(X=X,Z=residual[,ind],constantMean=1,parallel=TRUE)
}else{
fitPC = mlegp(X=X[-u,],Z=residual[-u,ind],constantMean=1,parallel=TRUE)}
sfStop()
id=length(ind)
pred=rep(0,id)
for(r in 1:id){
...
Gamma=Y2[T[[m]],]%*%l_cereal ###for
x_i=t(Gamma[,1:4])%*%Y2[T[[m]],]
x_star=t(Gamma[,1:4])%*%(y_star-mean(y_star))/sd(y_star)
X=t(x_i)
X_star=t(x_star)
###############fit GP emulator
residual=t(E)
...
if(length(u)==0){
fitPC = mlegp(X=X,Z=residual[,ind],constantMean=1,parallel=TRUE)
#fitPC2 = mlegp(X=X,Z=residual[,ind[76:length(ind)]],constantMean=1,parallel=TRUE)
