...
colnames(dat)=c("scld" ,"wcld","spcld" ,"acld" ,"spre","wpre" ,"sppre" ,"apre" ,"stmp" ,"wtmp" ,"sptmp","atmp","swet",   "wwet" ,"spwet","awet","iscld","iwcld","ispcld","iacld","ispre" ,"iwpre" ,  "isppre" , "iapre",  "istmp" ,  "iwtmp" ,"isptmp", "iatmp",  "iswet","iwwet","ispwet" ,"iawet")
load("rainfed2/other_inp")
...
source("rainfed2/co2_file.R",echo=TRUE)
...
clim=cm3
colnames(clim)=colnames(other_inp[[1]])
load("rainfed2/inni")
inn=array(inni,c(npixel,8,4))
wala=cbind(c(inn[,n,1]),c(inn[,n,2]),c(inn[,n,3]),c(inn[,n,4]))
colnames(wala)=colnames(inni)
dad=wala
colnames(dad)=c("cereal","rice","maize","oil")
s2=cbind(dad,wala,dat,clim,dad[,1])
s2=array(s2,c(npixel,length(n),45))


rm(au,cha,cld,clim,cm2,cm3,co2_9,co2_all,co2_b,co2_ini,wala,dad,dat,ind,inn,inp1,inp2,inp3,inp4,my_co2,new_co2,olu,other_inp,pre,s,tmp,sp,T,X1,X2,X2a,X2b,X2c,X2d,X3a,X3b,z,wet)
#########################end
