...
m=i
###################################################
nam=c("ini_cereal","ini_rice","ini_maize","ini_oil_max","scld" ,   "wcld"  ,  "spcld" ,  "acld" ,"spre",   
...
if(RC==5){
ini_cereal=input[,1]=rowMeans(ini[[1]][,1:4,man,fert])
ini_rice=input[,2]=rowMeans(ini[[2]][,1:4,man,fert])
ini_maize=input[,3]=rowMeans(ini[[3]][,1:4,man,fert])
ini_oil_max=input[,4]=rowMeans(ini[[4]][,1:4,man,fert])
} else {
ini_cereal=input[,1]=ini[[1]][,RC,man,fert]
ini_rice=input[,2]=ini[[2]][,RC,man,fert]
ini_maize=input[,3]=ini[[3]][,RC,man,fert]
ini_oil_max=input[,4]=ini[[4]][,RC,man,fert]
}
ini_crop=list(ini_cereal,ini_rice,ini_maize,ini_oil_max)
...
source("irrigated2/wls_irri.R",echo=F)
...
colnames(ini)=c("ini_cereal","ini_rice","ini_maize","ini_oil_max")
...
j=0
source("irrigated2/aggregate.R",echo=F)
} else {
...
j=0
source("irrigated2/net.R",echo=F)}
###############################END
} else {
if(fertilization==0){
fert=2
...
#################################################
#####include PCA analysis
source("irrigated2/wls_irri.R",echo=TRUE)
...
source("irrigated2/aggregate.R",echo=TRUE)
} else {
 #crop=crpp
#crop=array(crop,dim=c(59199,ncol(crop),1))
crop=cbind(crpp,ini)
crop=as.matrix(crop)
crop=array(crop,dim=c(59199,ncol(crop),1))
source("irrigated2/net.R",echo=TRUE)
}}
