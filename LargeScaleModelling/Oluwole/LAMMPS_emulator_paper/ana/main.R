...
design[,j]= qunif(hyper[,j], min=0, max=1)
}
tt=qunif(hyper[,23],min=min(time),max=max(time))###time
#design[,23]=tt
X1=data.frame(design[1:(npoints/2),])
X2=data.frame(design[((npoints/2)+1):npoints,])
npoints=1000
XX1=data.frame(matrix(runif(npoints*nvar),nrow=npoints))
XX2=data.frame(matrix(runif(npoints*nvar),nrow=npoints))
...
sens2bb=soboljansen(model=mm,X1=X1,X2=X2,nboot=1000,m=mod4)
sens2b=soboljansen(model=mod,X1=X1,X2=X2,nboot=1000)##linear fit
sens2c=soboljansen(model=mm,X1=X1,X2=X2,nboot=1000,m=mod3b)
fast=fast99(model =mm, factors =23, n = 1000,q = "qunif", q.arg = list(min = 0, max = 1))
dirname=getwd()
save(sens2a,file=paste(dirname,"sens2a",sep="/"))
save(sens2b,file=paste(dirname,"sens2b",sep="/"))
save(sens2c,file=paste(dirname,"sens2c",sep="/"))
...
ddd=cbind(x1,x2,x3)
par(mar=c(5.1,4.5,4,.5)+0.0)
xvals=barplot(t(ddd),ylim=c(0,2.5),
,ylab="Total sensitivity",main="Sensitivity analysis for floc mean diameter",offset=0,cex.lab=1.3,col=c("green","blue","pink"),density=c(40,60,90),cex.main=1.5,space=0.3,angle=c(45,90,135),cex.axis=1.5,cex.names=1.5,args.legend=list(x=c(1,10),y=c(1,1.5),cex=1.5),legend=c("Linear trend + Gaussian cov","Standard OLS","Linear trend + matern5_2 cov"))
text(xvals,par("usr")[3]-.03,srt=45,adj=1,labels=para,xpd=TRUE,cex=1.5)
