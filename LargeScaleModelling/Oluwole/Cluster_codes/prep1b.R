...
ind2=as.numeric(con2)
ind=length(con)
cnam=c("type","diameter","x","y","z")
sele=c("diameter","x","y","z")
index=con
dd=list()
index=as.numeric(c(index,ind2+9))
#k0=min(64,length(index))
k0=length(index)
library(doParallel)
cl=makeCluster(5)
registerDoParallel(cl)
dd=foreach(i=1:(k0-1),.verbose=T,.errorhandling="stop") %do% {
library(data.table)
