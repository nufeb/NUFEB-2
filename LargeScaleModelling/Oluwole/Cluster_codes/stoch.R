...
#para2=c("fix d1 all divide 16 v_EPSdens 1 3123","fix e1 HET eps_extract 16 v_EPSratio v_EPSdens 1242242")
para2=c("fix d1 all divide 100 v_EPSdens v_divMass 31231","fix e1 HET eps_extract 100 v_EPSratio v_EPSdens 12425")
val2=c(31231,12425)
set.seed(3)
nre2=npoints2
...
for(j in 1:nvar2){
design2[,j]=round(qunif(hyper2[,j], min=0.50*val2[j], max=1.5*val2[j]))
}
text2 <- readLines( gsub("fold",fold,paste("Inputscript", ".lammps",sep="")),encoding="UTF-8")
...
for( re2 in 1: nre2){
file.copy(c(sub("re2",re2,paste("Inputscriptre2", ".lammps",sep="")),"atom.in"), sub("re2",re2,paste("input", "re2",sep="")))
}
for(re2 in 1: nre2){
file.remove(sub("re2",re2,paste("Inputscriptre2", ".lammps",sep="")))
}
for(re2 in 1: nre2){
setwd(sub("re2",re2,paste("inputre2")))
file.rename(sub("re2",re2,paste("Inputscriptre2", ".lammps",sep="")),"Inputscript.lammps")
setwd("../")
}
setwd("..")
#######################end
