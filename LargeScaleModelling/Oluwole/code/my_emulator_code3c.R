...
kala=jitter(Obs,amount=1e-6)#####to prevent numerical problems
################my GP method
...
###########standardized coefficient from linear model: 09-10-2015
function (m1) 
{
    b <- summary(m1)$coef[-1, 1]
    sx <- sd(m1$model[-1])
    sy <- sd(m1$model[1])
    beta <- b * sx/sy
    return(beta)
