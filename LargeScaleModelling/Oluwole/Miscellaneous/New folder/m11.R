...
imp1=c(15,3,12,13,1)##c(D,eqv.dia,fractal,nparticle,mass)##interchange
...
noise[,c(4:5)]=log(noise[,c(4:5)])
h2=replace(noise,which(noise<0),0)
...
nug=list()
for(i in 1:nout){
nug[[i]]=h2[s,i]*pow[i]^2####noise variance/nugget values
}
##
...
rm(bb1,bb2,bb3,bb4,bb5,noise,output,X,Y,Z,alpha1,alpha2,alpha3)

library(doParallel)
cl=makeCluster(nout)
registerDoParallel(cl)
dirname=paste(getwd(),"result11",sep="/")
...
mymodel2=MuFicokm(formula=f,MuFidesign=nest,response=resp,nlevel=nout,control=list(multistart=nout),noise.var=nug,covtype="exp")##mean emulator
