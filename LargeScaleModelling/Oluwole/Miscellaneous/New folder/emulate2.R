...
des3=des3[-jj,]
des3=des3[-p,]
output=bb3[,imp2]#5 outputs
output1=(output[,1:5])/output[,6]
output=output1
s=sample(1:nrow(output),ndes)
out=matrix(NA,nrow=length(s),ncol=nout)
ini=out
bb4a=bb4[,imp2]###choose initial for the 5 outputs
bb4a1=(bb4a[,1:5])/bb4a[,6]
bb4a=bb4a1
...
noise=bb5[,imp2]######select the noise variance for just the 5 choosen outputs
noise1=noise[,1:5]/(rowSums(noise[,1:5])+1)##add small value to avoid dividing by zero
noise=noise1
...
#dirname=getwd()
rm(bb1,bb2,bb3,bb4,bb5,noise,nois,output)
#library(doParallel)
#cl=makeCluster(nout)
#registerDoParallel(cl)
dirname=paste(getwd(),"result2",sep="/")
...
mymodel2=MuFicokm(formula=f,MuFidesign=nest,response=resp,nlevel=nout,covtype="gauss",control=list(multistart=nout),noise.var=nug)##mean emulator
