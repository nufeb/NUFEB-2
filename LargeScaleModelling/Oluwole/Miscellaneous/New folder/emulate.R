...
#outputs=c("mass","BD2","eqvdiam","x","y","z","nHET","nAOB","nNOB","nEPs","nDEAD","fracdim","nparticle","segregindex","diversity","sub","o2","no2","o3","nh4")
...
imp1=c(1,3,12,13,15)##c(mass,eqv.dia,fractal,nparticle,D)
...
nug=list()
for(i in 1:nout){
nug[[i]]=r[,i]####noise variance/nugget values
}
##
...
#modd[[g]]=km(formula=f[[g]],nest$PX,response=resp[[g]],covtype="gauss",control=list(multistart=nout),noise.var=nug[[g]])
#library(doParallel)
#cl=makeCluster(nout)
#registerDoParallel(cl)
dirname=paste(getwd(),"result1",sep="/")
#mymodel1=MuFicokm(formula=f,MuFidesign=nest,response=nug,nlevel=nout,covtype="gauss",control=list(multistart=2*nout),nugget=sigs)##variance emulator
#prednoise=predict(mymodel1,newdata=nest$PX,type="UK")$mux
#save(mymodel1,file=paste(dirname,"mymodel1",sep="/"))###spa
...
mymodel2=MuFicokm(formula=f,MuFidesign=nest,response=resp,nlevel=nout,covtype="gauss",control=list(multistart=2),noise.var=nug)##mean emulator
mymodel2$cok[[2]]@AR.formula=~1;mymodel2$cok[[3]]@AR.formula=~1;mymodel2$cok[[4]]@AR.formula=~1;mymodel2$cok[[5]]@AR.formula=~1
mod=mymodel2
mod$ZD=NULL
mod$response=NULL
mod$Dnest=NULL
save(mod,file=paste(dirname,"mod",sep="/"))###spa
##kriging mpodels
modd=list()
for(g in 1:nout){
modd[[g]]=km(formula=f[[g]],nest$PX,response=resp[[g]],covtype="gauss",control=list(multistart=2),noise.var=nug[[g]])
}
