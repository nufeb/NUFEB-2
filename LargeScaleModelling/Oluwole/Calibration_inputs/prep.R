...
nt=(1-bv)*nn[t]
dt=nn[t]*ss[t]
VT=(bv*VT)+rgamma(1,shape=nt/2,dt/2)	
V[t]=1/VT	
}
V[1:p]=V[p+1]
V=V
...
for(t in (p+1):(p-1)){
Err[p+1,]=arx[p+1,]-t(Theta[,(p+1)])%*%F#arx[p:1,] #updated error
}
Thetai=Theta
Erri=Err
...
newoutput[t,]=  mvrnorm(1,mut,diag(sqrt(sigma2t)))
}
}
