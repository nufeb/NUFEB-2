...
void Set::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    ...
    if (strcmp(arg[iarg],"type") == 0) {
      ...
      if (strcmp(arg[iarg+1],"NULL") == 0) dvalue = -1.0;
      else if (utils::strmatch(arg[iarg+1],"^v_")) varparse(arg[iarg+2],2);
      else iyy = utils::numeric(FLERR,arg[iarg+2],false,lmp);
      if (strcmp(arg[iarg+1],"NULL") == 0) dvalue = -1.0;
      else if (utils::strmatch(arg[iarg+1],"^v_")) varparse(arg[iarg+3],3);
      else izz = utils::numeric(FLERR,arg[iarg+3],false,lmp);
      if (strcmp(arg[iarg+1],"NULL") == 0) dvalue = -1.0;
      else if (utils::strmatch(arg[iarg+1],"^v_")) varparse(arg[iarg+4],4);
      else ixy = utils::numeric(FLERR,arg[iarg+4],false,lmp);
      if (strcmp(arg[iarg+1],"NULL") == 0) dvalue = -1.0;
      else if (utils::strmatch(arg[iarg+1],"^v_")) varparse(arg[iarg+5],5);
      else ixz = utils::numeric(FLERR,arg[iarg+5],false,lmp);
      if (strcmp(arg[iarg+1],"NULL") == 0) dvalue = -1.0;
      else if (utils::strmatch(arg[iarg+1],"^v_")) varparse(arg[iarg+6],6);
...
void Set::set(int keyword)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    if (keyword == TYPE) {
      ...
      atom->rmass[i] = dvalue;
      // NUFEB specific
      if (atom->outer_mass_flag) atom->outer_mass[i] = dvalue;
    }
    else if (keyword == DIAMETER) {
      if (dvalue < 0.0) error->one(FLERR,"Invalid diameter in set command");
      atom->radius[i] = 0.5 * dvalue;
      // NUFEB specific
      if (atom->bacillus_flag) avec_bacillus->set_diameter(i,dvalue);
      if (atom->outer_radius_flag) atom->outer_radius[i] = 0.5 * dvalue;
    }
