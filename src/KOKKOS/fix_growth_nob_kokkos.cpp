...
void FixGrowthNOBKokkos<DeviceType>::update_cells()
{
  ...
  Kokkos::parallel_for(
    Kokkos::RangePolicy<
    DeviceType,
    FixGrowthNOBCellsReactionTag>(0, grid->ncells), f);
  copymode = 0;
...
void FixGrowthNOBKokkos<DeviceType>::update_atoms()
{
  ...
  Kokkos::parallel_for(
    Kokkos::RangePolicy<
    DeviceType,
    FixGrowthNOBCellsGrowthTag>(0, grid->ncells), f);

  Kokkos::parallel_for(
    Kokkos::RangePolicy<
    DeviceType,
    FixGrowthNOBAtomsTag >(0, atomKK->nlocal), f);
  copymode = 0;
...
void FixGrowthNOBKokkos<DeviceType>::Functor::operator()(FixGrowthNOBCellsReactionTag, int i) const
{
...
template <class DeviceType>
KOKKOS_INLINE_FUNCTION
void FixGrowthNOBKokkos<DeviceType>::Functor::operator()(FixGrowthNOBCellsGrowthTag, int i) const
{
  double tmp1 = growth * d_conc(ino2, i) / (no2_affinity + d_conc(ino2, i)) * d_conc(io2, i) / (o2_affinity + d_conc(io2, i));
  double tmp2 = maintain * d_conc(io2, i) / (o2_affinity + d_conc(io2, i));
...
template <class DeviceType>
KOKKOS_INLINE_FUNCTION
void FixGrowthNOBKokkos<DeviceType>::Functor::operator()(FixGrowthNOBAtomsTag, int i) const
{
