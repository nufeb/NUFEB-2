...
enum{DOMAIN1,CONSTANT,VARIABLE};

/* ---------------------------------------------------------------------- */

ComputeNufebPressure::ComputeNufebPressure(LAMMPS *lmp, int narg, char **arg) :
...
ComputeNufebPressure::~ComputeNufebPressure()
{
  delete [] id_temp;
  delete [] vector;
  delete [] vptr;
...
void ComputeNufebPressure::init()
{
  boltz = force->boltz;
  nktv2p = force->nktv2p;
  ...
  if (vol_style == VARIABLE) {
    ...
    if (vol_index < 0) {
      ...
      for (int i = 0; i < ncompute; i++) {
		if (strcmp(modify->compute[i]->style,"nufeb/volume") == 0) {
		  cvol = static_cast<ComputeVolume *>(lmp->modify->compute[i]);
		  break;
		}
      }
      if (cvol == NULL)
      	  error->all(FLERR,"Compute volume does not exist");
     // error->warning(FLERR,"Variable name for fix pressure does not exist");
    }
  }
...
double ComputeNufebPressure::compute_scalar()
{
...
void ComputeNufebPressure::compute_vector()
{
...
void ComputeNufebPressure::virial_compute(int n, int ndiag)
{
  int i,j;
...
void ComputeNufebPressure::reset_extra_compute_fix(const char *id_new)
{
  delete [] id_temp;
