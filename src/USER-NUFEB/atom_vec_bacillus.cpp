...
void AtomVecBacillus::data_atom_bonus(int m, char **values)
{
  ...
  pole1[1] = values[7];
  pole1[2] = values[8];
  bonus->height = values[9];
  if (bonus->height < 0)
    error->one(FLERR, "Invalid height in Bacilli section of data file: height < 0");

  pole2[0] = atom->x[m][0] - pole1[0];
  pole2[1] = atom->x[m][1] - pole1[1];
  pole2[2] = atom->x[m][2] - pole1[2];

  bonus->diameter = values[10];
  if (bonus->diameter < 0)
