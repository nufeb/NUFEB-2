...
enum{TYPE,TYPE_FRACTION,MOLECULE,X,Y,Z,CHARGE,MASS,SHAPE,LENGTH,TRI,
     ...
     INAME,DNAME,OUTERMASS,OUTERRADIUS,BIOMASS,BIODENSITY};

#define BIG INT_MAX

/* ---------------------------------------------------------------------- */

void BioSet::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    ...
    } else if (strcmp(arg[iarg],"biodensity") == 0) {
        if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
        if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
        else dvalue = force->numeric(FLERR,arg[iarg+1]);
        if (!atom->rmass_flag)
...
void BioSet::set(int keyword)
{
  ...
  AtomVecBio *avec_bio = (AtomVecBio *) atom->style_match("bio");
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    else if (keyword == OUTERRADIUS) {
      if (dvalue < 0.0) error->one(FLERR,"Invalid outerradius in set command");
      avec_bio->outer_radius[i] = 0.5 * dvalue;
    }
    else if (keyword == BIOMASS) {
      if (dvalue < 0.0) error->one(FLERR,"Invalid biomass in set command");
      avec_bio->biomass[i] =dvalue;
    }
    else if (keyword == BIODENSITY) {
      if (dvalue < 0.0) error->one(FLERR,"Invalid biodensity in set command");
      avec_bio->biomass[i] = 4.0*MY_PI/3.0 *
...
void BioSet::setrandom(int keyword)
{
  int i;
...
void BioSet::topology(int keyword)
{
...
void BioSet::varparse(char *name, int m)
{
  varflag = 1;
