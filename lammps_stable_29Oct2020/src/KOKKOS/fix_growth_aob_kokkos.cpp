...
#include "fix_growth_aob_kokkos.h"
...
FixGrowthAOBKokkos<DeviceType>::FixGrowthAOBKokkos(LAMMPS *lmp, int narg, char **arg) :
  FixGrowthAOB(lmp, narg, arg)
{
  kokkosable = 1;
...
template <class DeviceType>
void FixGrowthAOBKokkos<DeviceType>::update_cells()
{
  ...
  Kokkos::parallel_for(
    Kokkos::RangePolicy<
    DeviceType,
    FixGrowthAOBCellsReactionTag>(0, grid->ncells), f);
  copymode = 0;
...
template <class DeviceType>
void FixGrowthAOBKokkos<DeviceType>::update_atoms()
{
  for (int i = 0; i < 3; i++) {
  ...
  Kokkos::parallel_for(
    Kokkos::RangePolicy<
    DeviceType,
    FixGrowthAOBCellsGrowthTag>(0, grid->ncells), f);

  Kokkos::parallel_for(
    Kokkos::RangePolicy<
    DeviceType,
    FixGrowthAOBAtomsTag >(0, atomKK->nlocal), f);
  copymode = 0;
...
template <class DeviceType>
FixGrowthAOBKokkos<DeviceType>::Functor::Functor(FixGrowthAOBKokkos<DeviceType> *ptr):
  igroup(ptr->igroup), dt(ptr->dt), groupbit(ptr->groupbit),
  inh4(ptr->inh4), io2(ptr->io2), ino2(ptr->ino2),
  o2_affinity(ptr->o2_affinity), nh4_affinity(ptr->nh4_affinity),
...
void FixGrowthAOBKokkos<DeviceType>::Functor::operator()(FixGrowthAOBCellsReactionTag, int i) const
{
...
template <class DeviceType>
KOKKOS_INLINE_FUNCTION
void FixGrowthAOBKokkos<DeviceType>::Functor::operator()(FixGrowthAOBCellsGrowthTag, int i) const
{
  double tmp1 = growth * d_conc(inh4, i) / (nh4_affinity + d_conc(inh4, i)) * d_conc(io2, i) / (o2_affinity + d_conc(io2, i));
  double tmp2 = maintain * d_conc(io2, i) / (o2_affinity + d_conc(io2, i));
...
template <class DeviceType>
KOKKOS_INLINE_FUNCTION
void FixGrowthAOBKokkos<DeviceType>::Functor::operator()(FixGrowthAOBAtomsTag, int i) const
{
...
namespace LAMMPS_NS {
template class FixGrowthAOBKokkos<LMPDeviceType>;
#ifdef KOKKOS_ENABLE_CUDA
template class FixGrowthAOBKokkos<LMPHostType>;
#endif
}
